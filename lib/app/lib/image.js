...
class Image {
	...
	start() {
		...
		return this.processes.image.start();
	...
	stop() {
		...
		return this.processes.image.stop();
	...
	runScript(name) {
		let script = this.config.scripts[name];
	...
	_configure(config) {
		...
		this.config = {
			...
			scripts: this.pkg && this.pkg.scripts || {}
		};


		this.processes = {};
		this.processes.image = this._createImageProcess();
	...
	_createImageProcess() {
		...
		let cmd = [which.sync('nodemon'), '-x'].concat(
	...
	// 		that.log(['log'], utils.sanitizeProcessDataPayload(payload));
	// 	});

	// 	p.stderr.on('data', (payload) => {
	// 		that.log(['error'], utils.sanitizeProcessDataPayload(payload));
	// 	});
